...
cell_lines = set()
features = set()
lower_thr = options.thr.split(",") if options.thr else None
higher_thr = options.Thr.split(",") if options.Thr else None
...
#            element_id = "_".join((chr,start,end,strand,tags["splice_chain"].strip('"')))
			element_id = tags['event_id'].strip('"')
        else:
            element_id = tags['gene_id'].strip('"')
        if options.strip_id: element_id = element_id.split('.')[0]
        try:
            tags['iIDR'] = "0" if tags['iIDR'].strip('"') == "NA" else tags['iIDR'].strip('"')
        except KeyError:
            pass
        if lower_thr:
            for t in lower_thr:
...
#            d.setdefault(element_id, {}). setdefault(sample, ",".join(element_value_list))
            element_value = tags.get(options.value).strip('"')
            d.setdefault(element_id, {}).setdefault(sample,element_value)
