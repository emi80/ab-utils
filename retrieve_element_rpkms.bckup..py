...
parser.add_option('-e','--element', dest='element',help='choose the element you want to extract the matrix for: <gene>, <transcript>, <exon>, <intron>',metavar='')
parser.add_option('-n','--names', default=False, action='store_true', help='use it if you want the metadata as column header instead of labExpId')
parser.add_option('-r','--replicates', default=False, action='store_true', help='use this if you want to use only replicates')
parser.add_option('-v','--value', default='rpkm', help='choose the value you want to extract: <reads>, <RPKM>, <COSI>, <incRatio>', metavar='')
...
for line in sys.stdin:
	...
	for line in my_open(fname):
		...
		if options.replicates:
			...
			element_value = float(tags[options.value].strip('"')) if tags[options.value].strip('"') != "NA" else "NA"
...
for element in sorted(d.iterkeys()):
	values = d[element]
	f.write(element+'\t')
	f.write('\t'.join(str(values.get(cell_line,'NA')) for cell_line in sorted(cell_lines))+'\n')
f.close()
